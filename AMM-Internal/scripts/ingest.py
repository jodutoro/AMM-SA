...
def save_data_json(data: dict) -> None:
    ...
    DATA_JSON_PATH.write_text(json_str)
    # Also write data.js so the dashboard works when opened via file:// (no HTTP server)
    # data.js: const AMM_DATA format — consumed by index.html (and demo.html fallback)
    DATA_JS_PATH.write_text(
        "// ─── AMM Intelligence Dashboard — Data Layer ──────────────────────────────\n"
        "// Auto-generated by ingest.py — edit data.json or run ingest.py, not this file.\n"
        "// ─────────────────────────────────────────────────────────────────────────────\n\n"
        f"const AMM_DATA = {json_str};\n"
    )
...
def main():
    ...
    try:
        ...
        # Quick auth check — will raise if no key is resolvable
        _ = client.api_key
    except Exception as e:
        print(f"[error] ANTHROPIC_API_KEY not found in environment or .env: {e}")
        sys.exit(1)
